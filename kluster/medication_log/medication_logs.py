...
# @celery.task
def create_medication_logs_async(*args):
    user_id, medication_id = args
    ...
    for day in range(duration.days + 1):  # +1 to include the end date
        ...
        for dose in range(patient_dosage.get(medication.dosage, 1)):
            time_increment = timedelta(hours=24 / patient_dosage.get(medication.dosage, 1))
